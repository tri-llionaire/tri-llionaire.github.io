import definitions#add # for checkmate; enpassant into check; reimplement enpassant, castling, promotion, threefold repetition
...
y = 0; m = 0; enpassant = 0; checked = ''; last = 0
dictstate = dict(zip(positions, pieces))
while True:
    definitions.output(dictstate, wtaken, btaken)
    print(document)
    y += 1
    turn = y % 2
    if turn == 1:
        ...
        mine = list(black)
    possible = definitions.generate(dictstate, mine, enpassant)
    possible = definitions.clean(dictstate, possible, positions, mine)
    notyet = 0
    for x in possible:
        if possible[x] != []:
            notyet = 1
    if notyet == 0:
        if turn == 1:
            if definitions.iswhiteincheck(dictstate) == '+':
                print('black wins')
            else:
                print('stalemate')
        else:
            if definitions.isblackincheck(dictstate) == '+':
                print('white wins')
            else:
                print('stalemate')
        break
    ...
    if move == '0-0':
        ...
        if turn == 1:
            ...
            if toloc in possible[fromloc]:
                ...
                dictstate[fromloc] = '-'
                checked = definitions.isblackincheck(dictstate)
                if last == 1:
                    if checked != '':
                        print('you win')
                        break
            ...
            if toloc in possible[fromloc]:
                ...
                dictstate[toloc] = bufferpiece
                dictstate[fromloc] = '-'
                checked = definitions.iswhiteincheck(dictstate)
                if last == 1:
                    if checked != '':
                        print('you win')
                        break
